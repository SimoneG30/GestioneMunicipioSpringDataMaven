...
public class ExecuteModificaUtenteServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		Long idUtenteParsato = null;
		String idUtenteModifica = null;
		...
		if (!idUtente.isEmpty()) {
			idUtenteParsato = Long.parseLong(idUtente);
			idUtenteModifica = idUtenteParsato.toString();
		}
		Utente utenteAggiornato = utenteService.caricaSingoloUtente(idUtenteParsato);
		utenteAggiornato.setNome(nomeInput);
		utenteAggiornato.setCognome(cognomeInput);
		utenteAggiornato.setUsername(usernameInput);
		utenteAggiornato.setPassword(passwordInput);
